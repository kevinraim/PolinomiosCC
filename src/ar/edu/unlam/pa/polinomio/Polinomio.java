package ar.edu.unlam.pa.polinomio;
...
public class Polinomio {
	...
	private double potenciaRecurs(double x, int n) {
		return n==1? x : x * potenciaRecurs(x, n-1);
	}
	...
	private double potenciaRecursPar(double x, int n) {
		return n==1? x : (n % 2 == 0 ? potenciaRecursPar(x*x, n/2) : x * potenciaRecursPar(x, n-1));
	}
	...
	public double evaluarHorner(double x) {
		...
		for(int i = 1; i<coeficientes.length && grado >=1; i++) {
